...
function onHamburgerClick() {
  if (window.innerWidth <= 768) {
    document.querySelector("nav.subnav ul").classList.toggle("open");
    ...
    <div className="boccia">
      <nav className="subnav">
        ...
        <ul className="ui container">
          ...
          <li onClick={() => onNavigationClick()}>
            <NavLink to={`${match.path}/kalender`}>boccia kalender</NavLink>
          </li>
        </ul>
      </nav>
      <div>
        <Switch>
          ...
          />
          <Route
            path={`${match.path}/scholencompetitie`}
            component={props => <Competitie league={1} {...props} />}
          />
          <Route
            path={`${match.path}/nederland`}
            component={props => <Competitie league={2} {...props} />}
          />
          <Route
            exact
            path={`${match.path}/kalender`}
            component={BocciaKalender}
          />
